...
function skrivDatafil(filename, o) {
  const jsonPath = getDataPath(filename);
  return writeJson(jsonPath, o);
}

function skrivBuildfil(filename, o, delimiter = "  ") {
  const jsonPath = getBuildPath(filename);
  return writeJson(jsonPath, o, delimiter);
}
...
function writeJson(filePath, o, delimiter, prettify = false) {
  ...
  const schemaFilename = filePath.replace(".json", ".schema.json");
  log.warn("#######", schemaFilename, fs.existsSync(schemaFilename));
  if (fs.existsSync(schemaFilename))
    schema.addSchemaLink(dokument, schemaFilename);
  const bytes = writeBinary(
    filePath,
    JSON.stringify(dokument, null, delimiter)
  );
