...
getBuildPath = (relPath, extension = ".json") => {
  ...
  return buildRoot + "/" + filename.replace(".test", "");
};

getDataPath = (relPath, extension = ".json") => {
  const filename = path.basename(relPath, path.extname(relPath)) + extension;
  return dataRoot + "/" + filename.replace(".test", "");
};
...
function writeJsodn(filePath, o, delimiter, prettify = false) {
  ...
  dokument.meta = {
    ...
    elementer: getLength(o.items)
  };
  if (meta) Object.assign(dokument.meta, meta);
  dokument.items = json.sortByKey(dokument.items);
