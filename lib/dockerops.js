...
function start_gdal(name, srcFolder) {
    ...
    let cmd = `docker run --name ${name} -di --restart unless-stopped -v ${hostPath}:/tmp:rw ghcr.io/osgeo/gdal:alpine-normal-latest`
...
function start_ogr_containerNImage(name) {// : void
    ...
    let cmd = `docker run --name ${name} -di --restart unless-stopped -v ${tempPath}:/tmp:rw ghcr.io/osgeo/gdal:alpine-normal-latest`
    this.run_in_shell(cmd);
    console.log(name);
    return name;
}
