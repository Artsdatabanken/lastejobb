...
class UrlGenerator {
  ...
  url(kode) {
    ...
    node.url = "/" + subUrl;
    if (this.prefix) node.url = "/" + this.prefix + node.url;
    if (node.foreldre.length > 0) {
      let parentUrl = this.url(node.foreldre[0]);
  ...
  urlify(tittel, kode, makevalid) {
    let s = tittel.sn || tittel.nob || tittel.nb; // TODO: Remove nb
    if (!s) {
